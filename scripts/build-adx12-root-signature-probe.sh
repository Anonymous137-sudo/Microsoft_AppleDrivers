...
[ "$#" -eq 2 ] || {
    printf 'usage: build-adx12-root-signature-probe.sh dxmt-source output.exe\n' >&2
    exit 64
}
...
source_file="$repo_root/Direct3D_12(Feature_Level_12_2)/tests/windows/root_signature_semantics.cpp"
...
"$compiler" \
    ...
    -I "$directx_include" \
    "$source_file" -o "$output" -lole32 -luuid
